var jsCalendar = (function () {
  ...
  JsCalendar.prototype._construct = function (args) {
    ...
    this._setDate(
      args.date !== null
        ? args.date
        : this._target.dataset.hasOwnProperty('date')
        ? this._target.dataset.date
        : new Date(),
    )
    // If invalid date
  ...
  JsCalendar.prototype._parseOptions = function (doptions) {
    ...
    if (
      ...
      typeof this.languages[options.language] !== 'undefined'
    ) {
    ...
    if (typeof options.firstDayOfTheWeek !== 'undefined') {
      ...
      if (typeof options.firstDayOfTheWeek === 'string') {
        ...
        if (options.firstDayOfTheWeek.match(/^[1-7]$/)) {
          this._options.firstDayOfTheWeek = parseInt(
            options.firstDayOfTheWeek,
            10,
          )
        }
        // else use it as a day name
          ...
          // So find day
          this._options.firstDayOfTheWeek =
            this.language.days.indexOf(options.firstDayOfTheWeek) + 1

          // Range check (no need to check for bigger then 7 but I don't trust anyone)
          if (
            this._options.firstDayOfTheWeek < 1 ||
            this._options.firstDayOfTheWeek > 7
          ) {
            this._options.firstDayOfTheWeek = 1
          }
        }
  ...
  JsCalendar.prototype._isDateInRange = function (date) {
    ...
    if (
      ...
      this._options.min.getTime() > date.getTime()
    ) {
      return false
    }
    // Check max
    if (
      ...
      this._options.max.getTime() < date.getTime()
    ) {
      return false
    }
  ...
  JsCalendar.prototype._parseToDateString = function (date, format) {
    ...
    return format.replace(
      /(MONTH|month|MMM|mmm|mm|m|MM|M|DAY|day|DDD|ddd|dd|d|DD|D|YYYY|yyyy)/g,
      function (key) {
  ...
  JsCalendar.prototype._getVisibleMonth = function (date) {
    ...
    // Get month's name
    var lang = this.language
    var name = this._options.monthFormat.replace(
      /(MONTH|month|MMM|mmm|##|#|YYYY|yyyy)/g,
      function (key) {
    ...
    if (
      first.getYear() === this._now.getYear() &&
      first.getMonth() === this._now.getMonth()
    ) {
      // Calculate current
  ...
  JsCalendar.prototype._create = function () {
    ...
    for (i = 0; i < 7; i++) {
      ...
      this._elements.headRows[1].appendChild(
        this._elements.days[this._elements.days.length - 1],
      )
    }
    ...
    for (i = 0; i < 6; i++) {
      ...
      for (j = 0; j < 7; j++) {
        ...
        this._elements.bodyRows[i].appendChild(
          this._elements.bodyCols[i * 7 + j],
        )
        this._elements.bodyCols[i * 7 + j].addEventListener(
          'click',
          (function (index) {
  ...
  JsCalendar.prototype._update = function () {
    ...
    if (month.current >= 0) {
      if (this._elements.bodyCols[month.current].className.length > 0) {
        this._elements.bodyCols[month.current].className +=
          ' jsCalendar-current'
      } else {
    ...
    for (i = 0; i < 7; i++) {
      var that = this
      this._elements.days[i].textContent = this._options.dayFormat.replace(
        /(DAY|day|DDD|ddd|DD|dd|D)/g,
        function (key) {
    ...
    if (this._events.month_render.length > 0) {
      ...
      for (j = 0; j < this._events.month_render.length; j++) {
        this._events.month_render[j].call(
          this,
          // Month index
          date.getMonth(),
    ...
    if (this._events.day_render.length > 0) {
      for (i = 0; i < 7; i++) {
        ...
        for (j = 0; j < this._events.day_render.length; j++) {
          this._events.day_render[j].call(
            this,
            // Day index
    ...
    if (this._events.date_render.length > 0) {
      for (i = 0; i < month.days.length; i++) {
        ...
        for (j = 0; j < this._events.date_render.length; j++) {
          this._events.date_render[j].call(
            this,
  ...
  JsCalendar.prototype.isVisible = function (date) {
    ...
    if (
      visible[0].getTime() <= date &&
      visible[visible.length - 1].getTime() >= date
    ) {
      return true
    }
    // Not visible
  ...
  JsCalendar.addLanguage = function (language) {
    ...
    language._dateStringParser = language.hasOwnProperty('dateStringParser')
      ? function (key, date) {
          return (
            language.dateStringParser(key, date) ||
            JsCalendar._defaultDateStringParser(key, date, language)
          )
        }
      : function (key, date) {
          return JsCalendar._defaultDateStringParser(key, date, language)
        }
    language._dayStringParser = language.hasOwnProperty('dayStringParser')
      ? function (key, day) {
          return (
            language.dayStringParser(key, day) ||
            JsCalendar._defaultDayStringParser(key, day, language)
          )
        }
      : function (key, day) {
